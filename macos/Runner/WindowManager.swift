...
class WindowManagerPlugin: NSObject, FlutterPlugin {
    static var windowControllers: Dictionary<String, NSWindowController> = [:]
    ...
    func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case "showWindow":
            let args = call.arguments as? [String: Any]
            let key: String! = args?["key"] as? String
            result(showWindow(_key: key))
            break
        case "hideWindow":
            let args = call.arguments as? [String: Any]
            let key: String! = args?["key"] as? String
            result(hideWindow(_key: key))
            break
        default:
    ...
    func createNewWindow(key: String, x: Double? = nil, y: Double? = nil, width: Int? = nil, height: Int? = nil) {
        ...
        WindowManagerPlugin.windowControllers[key] = windowController
    }
    ...
    func createWebWindow(key: String, url: String, jsMessage: String, x: Double? = nil, y: Double? = nil, width: Int? = nil, height: Int? = nil, result: @escaping FlutterResult) {
        ...
        windowController.window = window
        windowController.showWindow(self)
        NSApp.context?.setValue(windowController, forKey: key)
    }
    ...
    func hideWindow(_key: String) -> Bool {
        let window = NSApp.windows.first(where: { $0.title == _key })
        window?.orderOut(nil)
        return true
    }

    func showWindow(_key: String) -> Bool {
        let windowController = WindowManagerPlugin.windowControllers[_key]
